<template lang="">
  <div class="p-1 mx-auto">
    <Head>
      <Title>Blue Raccoon - Solve & Simplify</Title>
      <Meta
        name="Our process for software development, implementation and maintanance"
        content="Solve and simplify complex software problems, Our process" />
    </Head>

    <div class="mx-auto flex justify-center">
      <div class="flex justify-center mx-auto wrap">
        <div
          class="block max-w-sm rounded-lg shadow-lg m-5 bg-white mx-10 my-14">
          <div class="srv-1"></div>
          <div class="p-6">
            <h5 class="mb-2 text-xl font-medium leading-tight text-neutral-800">
              Process, Content and Security Technology
            </h5>
            <p class="mb-4 text-base text-neutral-600">
              Middleware to enhance value and function of existing systems &
              Software to manage, deliver multi-system content to users via
              integrated custom UI's
            </p>
            <a
              href="https://outlook.office365.com/owa/calendar/consultblueracccooncoza@blueraccoon.co.za/bookings/s/fW1JhTF9X0GtGqrAfPxOpA2">
              <button
                type="button"
                class="inline-block rounded bg-orange-500 px-6 pt-2.5 pb-2 text-xs font-medium uppercase leading-normal text-white shadow-[0_4px_9px_-4px_#3b71ca] transition duration-150 ease-in-out hover:bg-orange-600 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:bg-orange-600 focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:outline-none focus:ring-0 active:bg-orange-700 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)]"
                data-te-ripple-init
                data-te-ripple-color="light">
                Request free consoltasion
              </button>
            </a>
          </div>
        </div>
        <div
          class="block max-w-sm rounded-lg shadow-lg m-5 bg-white mx-10 my-14">
          <div class="srv-2"></div>
          <div class="p-6">
            <h5 class="mb-2 text-xl font-medium leading-tight text-neutral-800">
              Consulting & Advisory Services
            </h5>
            <p class="mb-4 text-base text-neutral-600">
              BPM - Optimization/Re-engineering <br />
              Content Management Strategy <br />
              System Migration Consulting <br />
              Scalable technology Strategies <br />
              Change Management
            </p>
            <a
              href="https://outlook.office365.com/owa/calendar/consultblueracccooncoza@blueraccoon.co.za/bookings/s/fW1JhTF9X0GtGqrAfPxOpA2">
              <button
                type="button"
                class="inline-block rounded bg-orange-500 px-6 pt-2.5 pb-2 text-xs font-medium uppercase leading-normal text-white shadow-[0_4px_9px_-4px_#3b71ca] transition duration-150 ease-in-out hover:bg-orange-600 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:bg-orange-600 focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:outline-none focus:ring-0 active:bg-orange-700 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)]"
                data-te-ripple-color="light">
                Request free consoltasion
              </button>
            </a>
          </div>
        </div>
        <div
          class="block max-w-sm rounded-lg shadow-lg m-5 bg-white mx-10 my-14">
          <div class="srv-3"></div>
          <div class="p-6">
            <h5 class="mb-2 text-xl font-medium leading-tight text-neutral-800">
              Technical Services
            </h5>
            <p class="mb-4 text-base text-neutral-600">
              Solution Design/Architecture <br />
              Process and Data Mapping <br />
              Technical Scoping & Project Management Change Management <br />
              User Acceptance Testing
            </p>
            <a
              href="https://outlook.office365.com/owa/calendar/consultblueracccooncoza@blueraccoon.co.za/bookings/s/fW1JhTF9X0GtGqrAfPxOpA2">
              <button
                type="button"
                class="inline-block rounded bg-orange-500 px-6 pt-2.5 pb-2 text-xs font-medium uppercase leading-normal text-white shadow-[0_4px_9px_-4px_#3b71ca] transition duration-150 ease-in-out hover:bg-orange-600 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:bg-orange-600 focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:outline-none focus:ring-0 active:bg-orange-700 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)]"
                data-te-ripple-color="light">
                Request free consoltasion
              </button>
            </a>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<style lang="css">
.srv-1 {
  background-image: url(../assets/8906821a6dfd4171341caa9e6d71fea1.jpg);
  padding: 10rem;
  background-size: cover;
  background-position: center;
  margin: 1rem;
  border-radius: 5px;
  border: 2px solid;
}

.srv-2 {
  background-image: url(../assets/18c896df2e453bd8073ac52a00146100.jpg);
  padding: 10rem;
  background-size: cover;
  background-position: center;
  margin: 1rem;
  border-radius: 5px;
  border: 2px solid;
}

.srv-3 {
  background-image: url(../assets/6c3663c1a30c599ee62238937c60219a.jpg);
  padding: 10rem;
  background-size: cover;
  background-position: center;
  margin: 1rem;
  border-radius: 5px;
  border: 2px solid;
}

@media only screen and (max-width: 1250px) {
  .wrap {
    flex-direction: column;
  }
}
</style>
